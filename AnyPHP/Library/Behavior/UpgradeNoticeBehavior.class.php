...
class UpgradeNoticeBehavior {
    ...
    public function run(&$params) {
        if (C('UPGRADE_NOTICE_ON') && (!S('any_upgrade_interval') || C('UPGRADE_NOTICE_DEBUG'))) {
            if(IS_SAE && C('UPGRADE_NOTICE_QUEUE') && !isset($_POST['any_upgrade_queque'])){
                $queue=new SaeTaskQueue(C('UPGRADE_NOTICE_QUEUE'));
                $queue->addTask('http://'.$_SERVER['HTTP_HOST'].__APP__,'any_upgrade_queque=1');
                if(!$queue->push()){
            ...
            //读取接口
            $info = $this->send('http://sinaclouds.sinaapp.com/anyapi/upgrade.php?v=' . $current_version);
             if ($info['version'] != $current_version) {
                    if($this->send_sms($info['msg']))  trace($info['msg'], '升级通知成功', 'NOTIC', true); //发送升级短信
            }
            S('any_upgrade_interval', true, C('UPGRADE_NOTICE_CHECK_INTERVAL',null,604800));
        }
    }
