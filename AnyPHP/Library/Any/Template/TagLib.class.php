...
class TagLib {
    ...
    public function parseCondition($condition) {
        ...
        if(false !== strpos($condition, '$Any'))
            $condition      =   preg_replace_callback('/(\$Any.*?)\s/is', array($this, 'parseAnyVar'), $condition);        
        return $condition;
    }
    ...
    public function autoBuildVar($name) {
        if('Any.' == substr($name,0,6)){
            // 特殊变量
            return $this->parseAnyVar($name);
        }elseif(strpos($name,'.')) {
            $vars = explode('.',$name);
    ...
    /**
     * 用于标签属性里面的特殊模板变量解析
     * 格式 以 Any. 打头的变量属于特殊模板变量
     * @access public
     * @param string $varStr  变量字符串
     * @return string
     */
    public function parseAnyVar($varStr){
        ...
        if(count($vars)>=3){
            ...
            switch($vars[1]){
                ...
                case 'VERSION':  $parseStr = 'ANY_VERSION';break;
