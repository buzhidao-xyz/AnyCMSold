    ...
    static public function dispatch() {
        ...
        if(C('APP_SUB_DOMAIN_DEPLOY')) {
            ...
            if(!empty($rule)) {
                ...
                $array      =   explode('/',$rule);
                // 模块绑定
                define('MAIN_MODULE',array_shift($array));
                // 控制器绑定         
                if(!empty($array)) {
                    $controller  =   array_shift($array);
                    if($controller){
                        define('MAIN_CONTROLLER',$controller);
                    }
                }
                if(isset($vars)) { // 传入参数
        ...
        if( MODULE_NAME && (defined('MAIN_MODULE') || !in_array_case(MODULE_NAME,C('MODULE_DENY_LIST')) ) && is_dir(APP_PATH.MODULE_NAME)){
            ...
            if(is_file(MODULE_PATH.'Conf/alias.php'))
                Any::addMap(include MODULE_PATH.'Conf/alias.php');
            // 加载模块tags文件定义
        ...
        define('__MODULE__',(defined('MAIN_MODULE') || !C('MULTI_MODULE'))? __APP__ : __APP__.'/'.($urlCase ? strtolower($moduleName) : $moduleName));
        ...
        if('' != $_SERVER['PATH_INFO'] && (!C('URL_ROUTER_ON') ||  !Route::check()) ){   // 检测路由规则 如果没有则按默认规则调度URL
            ...
            if(!defined('MAIN_CONTROLLER')) {// 获取控制器
        ...
        // 获取控制器和操作名
        define('CONTROLLER_NAME',   defined('MAIN_CONTROLLER')? MAIN_CONTROLLER : self::getController($varController,$urlCase));
        define('ACTION_NAME',       defined('BIND_ACTION')? BIND_ACTION : self::getAction($varAction,$urlCase));

        // 当前控制器的UR地址
        $controllerName    =   defined('CONTROLLER_ALIAS')? CONTROLLER_ALIAS : CONTROLLER_NAME;
        define('__CONTROLLER__',__MODULE__.$depr.(defined('MAIN_CONTROLLER')? '': ( $urlCase ? parse_name($controllerName) : $controllerName )) );
