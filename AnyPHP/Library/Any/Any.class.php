...
class Any
{
    ...
    {
		// 注册AUTOLOAD方法
		spl_autoload_register('Any\Any::autoload');
		// 设定错误和异常处理
		register_shutdown_function('Any\Any::fatalError');
		set_error_handler('Any\Any::appError');
		set_exception_handler('Any\Any::appException');
		...
		if(!APP_DEBUG && Storage::has($runtimefile)){
			...
			if(!APP_DEBUG){
			  ...
			  $content  .=  "\nL(".var_export(L(),true).");\nC(".var_export(C(),true).');Any\Hook::import('.var_export(Hook::get(),true).');}';
    ...
    static public function halt($error) {
        ...
        // 包含异常页面模板
        $exceptionFile =  C('TMPL_EXCEPTION_FILE',null,ANY_PATH.'Tpl/exception.hml');
     ...
     * @return void|array
     */
    static public function trace($value='[any]',$label='',$level='DEBUG',$record=false) {
        static $_trace =  array();
        if('[any]' === $value){ // 获取trace信息
            return $_trace;
        }else{
